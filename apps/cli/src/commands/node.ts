...
export function registerNodeCommands(program: Command): void {
  ...
  node
    ...
    .action(async (file: string, cmdOpts: { type?: string; name?: string }) => {
      ...
      try {
        ...
        // Upload the encoded node via client (auto-handles access token)
        const result = await resolved.client.nodes.put(nodeKey, encoded.bytes);
        if (!result.ok) {
  ...
  node
    .command("claim <key>")
    .description("Claim ownership of a node via Proof-of-Possession")
    .requiredOption("--pop <pop>", "proof-of-possession string (format: pop:XXXX...)")
    .action(async (key: string, cmdOpts: { pop: string }) => {
      const opts = program.opts();
      const formatter = createFormatter(opts);
      ...
      try {
        ...
        const result = await resolved.client.nodes.claim(key, cmdOpts.pop);
        if (!result.ok) {
          formatter.error(`Failed to claim node: ${result.error.message}`);
          process.exit(1);
        }

        const claim = result.data;
        formatter.output(claim, () => {
          const lines = [
            `Node:        ${claim.nodeHash}`,
            `Delegate:    ${claim.delegateId}`,
            `Already Own: ${claim.alreadyOwned ? "yes" : "no"}`,
          ];
          return lines.join("\n");
        });
      } catch (error) {
