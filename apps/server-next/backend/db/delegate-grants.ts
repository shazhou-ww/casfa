...
export type DelegateGrantStore = {
  ...
  getByRefreshTokenHash(realmId: string, refreshTokenHash: string): Promise<DelegateGrant | null>;
...
export function createMemoryDelegateGrantStore(): DelegateGrantStore {
  ...
  const byRealmAndRefreshHash = new Map<string, DelegateGrant>();
  ...
  return {
    ...
    async getByRefreshTokenHash(realmId: string, refreshTokenHash: string) {
      return byRealmAndRefreshHash.get(realmHashKey(realmId, refreshTokenHash)) ?? null;
    },
    async insert(grant: DelegateGrant) {
      ...
      if (grant.refreshTokenHash) {
        byRealmAndRefreshHash.set(realmHashKey(grant.realmId, grant.refreshTokenHash), grant);
      }
    },
    async remove(delegateId: string) {
      ...
      if (g) {
        ...
        if (g.refreshTokenHash) {
          byRealmAndRefreshHash.delete(realmHashKey(g.realmId, g.refreshTokenHash));
        }
      }
    },
