...
function App() {
  ...
  const cognitoUrl = document.querySelector<HTMLMetaElement>(
    'meta[name="cognito-hosted-ui-url"]'
  )?.content;
  const clientId = document.querySelector<HTMLMetaElement>(
    'meta[name="cognito-client-id"]'
  )?.content;
      ...
      >
        <span
          style={{
            ...
            borderRadius: "50%",
            background:
              status === "online" ? "#22c55e" : status === "offline" ? "#ef4444" : "#eab308",
          }}
        />
      ...
      <div>
        {loginUrl ? (
          ...
          <p style={{ fontSize: 13, color: "#999" }}>
            Cognito not configured — set meta tags or config endpoint.
          </p>
        )}
      </div>
    </div>
