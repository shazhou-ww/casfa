/**
 * SSO: login redirect, logout, .well-known, and OAuth stub routes. No token/callback on this cell.
 */
...
const PENDING_CLIENT_TTL_SEC = 3600;
...
export function createLoginRedirectRoutes(
  ...
  routes.get("/.well-known/oauth-authorization-server", (c) => {
    ...
    return c.json({
      ...
      scopes_supported: ["use_mcp", "manage_delegates"],
    });
  });
