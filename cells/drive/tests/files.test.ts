...
describe("Files (User)", () => {
  ...
  it("list root returns 200 and entries array", async () => {
    ...
    const res = await ctx.helpers.authRequest(token, "GET", `/api/realm/${realmId}/files`);
  ...
  it("upload file then stat and get content", async () => {
    ...
    const getRes = await fetch(`${ctx.baseUrl}/api/realm/${realmId}/files/bar.txt`, {
  ...
  it("stat root (directory) returns kind directory", async () => {
    const token = await ctx.helpers.createUserToken(realmId);
    const res = await ctx.helpers.authRequest(token, "GET", `/api/realm/${realmId}/files?meta=1`);
    expect(res.status).toBe(200);
  ...
  it("rm removes entry then list does not show it", async () => {
    ...
    const rmRes = await ctx.helpers.authRequest(token, "POST", `/api/realm/${realmId}/fs/rm`, {
      path: "toRemove",
    });
  ...
  it("mv moves file then get at new path returns content", async () => {
    ...
    const mvRes = await ctx.helpers.authRequest(token, "POST", `/api/realm/${realmId}/fs/mv`, {
      from: "dirA/origin.txt",
      to: "dirA/dest.txt",
    });
    expect(mvRes.status).toBe(200);
    const getRes = await fetch(`${ctx.baseUrl}/api/realm/${realmId}/files/dirA/dest.txt`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(getRes.status).toBe(200);
  ...
  it("cp copies file then get at new path returns same content", async () => {
    ...
    const cpRes = await ctx.helpers.authRequest(token, "POST", `/api/realm/${realmId}/fs/cp`, {
      from: "source.txt",
      to: "copy.txt",
    });
    ...
    const getSource = await fetch(`${ctx.baseUrl}/api/realm/${realmId}/files/source.txt`, {
      headers: { Authorization: `Bearer ${token}` },
    });
