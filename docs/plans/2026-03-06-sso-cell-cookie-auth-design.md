...
| 包名 | 使用方 | 职责 |
|------|--------|------|
| **cell-cognito-server** | SSO **后端** | 基于 Cognito 的登录：JWT 校验、code 换 token、refresh。若未来有多个根域名分别作 SSO，各 SSO 后端仍用此包对接各自 Cognito。（原 cell-cognito。） |
| **cell-cognito-webui** | SSO **前端** | 公共登录 UI 组件（如登录页、同意页），供 SSO cell 前端渲染。 |
| **cell-auth-server** | **业务 cell 后端**、**SSO 后端** | 签发 CSRF cookie、鉴权：从 Cookie 读 access token、校验 JWT、校验 CSRF（double submit）。含 getTokenFromRequest、buildAuthCookieHeader、buildRefreshCookieHeader、getCookieFromRequest、CSRF 的 generate/get/validate/build 等。SSO 后端写/清 access·refresh cookie 时也可复用 cell-auth-server 的 cookie 构建与清除。 |
...
| 包名 | 使用方 | 职责 |
|------|--------|------|
| **cell-delegates-server** | **业务 cell 后端** | 将现有 delegate 管理服务端逻辑迁入：创建/撤销/列举 delegate、consent、token 轮转等。 |
| **cell-delegates-webui** | **业务 cell 前端** | 将现有 delegate 管理的前端共享组件与逻辑迁入。 |
| **cell-delegates-client** | **业务 client** | 封装 delegate 管理相关 API 的调用。 |

### 4.3 业务 cell 后端（使用 cell-auth-server）

- **鉴权**：使用 **cell-auth-server** 的 `getTokenFromRequest(c.req.raw, { cookieName })`（仅从 Cookie 读），再调用与 SSO 一致的 JWT 校验得到 userId，设置 `c.set("auth", ...)`。
- **CSRF**：GET `/api/csrf` 用 cell-auth-server 的 `generateCsrfToken` + `buildCsrfCookieHeader` 写本域 csrf cookie；对写操作用 `validateCsrf(..., { headerName: "X-CSRF-Token" })`，不通过则 403。
...
- 2026-03-06：**包名与职责**：鉴权服务端拆为 `@casfa/cell-auth-server`（cookie/CSRF）；cell-oauth 依赖 cell-auth-server。Delegate 服务端拆为 `@casfa/cell-delegates-server`（createDelegatesRoutes）；设计稿 cell-auth → cell-auth-server，cell-delegates → cell-delegates-server。
- 2026-03-06：**Cognito 包**：SSO 后端共享包改名为 `@casfa/cell-cognito-server`（原 cell-cognito）；SSO 前端共享包 `@casfa/cell-cognito-webui`（CognitoLoginCard、buildCognitoLoginAuthorizeUrl），apps/sso 前端已接入。
