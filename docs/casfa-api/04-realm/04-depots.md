...
```
1. 读取文件
   GET /api/realm/{realmId}/nodes/fs/dpt_xxx/read?path=src/main.ts

2. 写入修改后的文件
   POST /api/realm/{realmId}/nodes/fs/dpt_xxx/write?path=src/main.ts
...
```
1. 第一次修改
   POST /api/realm/{realmId}/nodes/fs/dpt_xxx/write?path=file1.ts
   → newRoot: "nod_step1..."

2. 基于上一步的 newRoot 继续修改
   POST /api/realm/{realmId}/nodes/fs/nod_step1.../write?path=file2.ts
   → newRoot: "nod_step2..."
