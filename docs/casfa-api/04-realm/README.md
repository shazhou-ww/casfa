...
| 方法 | 路径 | 描述 | 认证 |
|------|------|------|------|
| GET | `/api/realm/{realmId}/nodes/fs/:key/stat` | 获取文件/目录元信息 | AT 或 JWT |
...
| 标志 | 影响的操作 |
|------|-----------|
| `canUpload` | 允许上传节点（`PUT /nodes/raw/:key`）、Claim 节点、Commit Depot、文件系统写操作 |
...
GET /api/realm/{realmId}/nodes/raw/nod_SCOPE_ROOT
Authorization: Bearer {access_token}

# 从 scope root 沿 ~N 导航
GET /api/realm/{realmId}/nodes/raw/nod_SCOPE_ROOT/~1/~2
Authorization: Bearer {access_token}
```
