...
| 方法 | 路径 | 描述 | 认证 |
|------|------|------|------|
| GET | `/.well-known/oauth-authorization-server/{cellMount}` | OAuth 2.1 授权服务器元数据 (RFC 8414, path issuer) | 无 |
| GET | `/.well-known/oauth-protected-resource` | 受保护资源元数据 (RFC 9728) | 无 |

> `GET /.well-known/oauth-authorization-server`（无 `{cellMount}` 后缀）返回 `404`。
...
## GET /.well-known/oauth-authorization-server/{cellMount}
...
```json
{
  "issuer": "https://example.com/agent",
  "authorization_endpoint": "https://example.com/agent/oauth/authorize",
  "token_endpoint": "https://example.com/agent/oauth/token",
  "registration_endpoint": "https://example.com/agent/oauth/register",
