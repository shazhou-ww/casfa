...
| 优先级 | 提取内容 | 目标位置 | 消除重复行数 |
|---|---|---|---|
| **P0** | Crockford Base32 encode/decode | `@casfa/encoding` (新建，零运行时依赖) | ~180 行 (3 处) |
| **P0** | `storage-utils.ts` (LRU + toStoragePath) | `@casfa/storage-core` | ~56 行 (2 处) |
| **P1** | `formatSize()` | `@casfa/encoding` 或独立工具包 | ~35 行 (3 处) |
| **P1** | PKCE 实现 | CLI 导入 `@casfa/client-auth-crypto` | ~60 行 (1 处) |
| **P1** | Base64URL encode/decode | `@casfa/encoding` | ~15 行 (3+ 处) |
| **P1** | 泛型 `prefixedIdToBytes` / `bytesToPrefixedId` | `@casfa/protocol` | ~40 行 |
| **P2** | `withExistsCache()` Storage 包装器 | `@casfa/storage-core` | ~60 行 (2 处) |
| **P2** | `waitForDynamoDB` 脚本工具 | `apps/server/backend/scripts/shared/` | ~40 行 (4–5 处) |
| **P2** | `Result<T, E>` 类型 | `@casfa/protocol` 或 `@casfa/result` | ~60 行 |
| **P2** | Blake3 哈希封装合并 | server 内合并 | ~30 行 |

**总计可消除约 ~580 行重复代码。**

---

## 建议新建包：@casfa/encoding

核心思路：创建一个零运行时依赖的编码工具包，解决最关键的循环依赖问题。

```
@casfa/encoding
├── src/
│   ├── crockford-base32.ts    # encodeCB32 / decodeCB32
│   ├── base64url.ts           # base64urlEncode / base64urlDecode
│   ├── hex.ts                 # bytesToHex / hexToBytes (从 core 迁移)
│   ├── format.ts              # formatSize / formatDuration
│   └── index.ts
├── package.json
└── tsconfig.json
```
...
这将彻底消除 `core` ↔ `protocol` 之间的循环依赖，并为未来的编码相关需求提供统一入口。
